        ...
        <div class="row">
            <div class="col-md-12">
                ...
                if(isset($_POST['submit'])){
                    ...
                    $query = "INSERT INTO `product`(`name`, `price`, `cat_id`,`parent_id`, `content`, `count`, `picture`) 
                                VALUES ('{$name}','{$price}','{$cat_id}','{$parent_id}','{$content}','{$count}','{$newFileName}')";
                    $result = $mysqli->query($query);
                    if($result){
                        header('location:index.php?msg=Thêm sản phẩm thành công');
                    }else{
                        echo "Lỗi khi thêm sản phẩm.Vui lòng thử lại!";
                    }
                }
                ?>
